 ...
 * The nodes are a non-static (instance) nested class.
 */
...
public class List2<T> implements Iterable<T> {
     ...
     */
    public static <T> void printList(List2<T> list) {
        List2<T>.Node nodeptr = list.getFirstNode();
        while (nodeptr != null) {
     ...
     * @param list the list to print
     */
    public static void printList2(List2<?> list) {
        List2<?>.Node nodeptr = list.getFirstNode();
        while (nodeptr != null) {
            System.out.println(nodeptr.getElement());
            nodeptr = nodeptr.getNext();
        }
     ...
     * As a nonstatic nested class, it does get the generic of the containing class.
     * We do not declare our own.
     */
    private class Node {
    ...
    public static void main(String[] args) {
        List2<Integer> list = new List2<Integer>();
        List2<Integer>.Node node = list.new Node(5, null);
        list.setFirstNode(node);
        LinkedList<JFrame> frames = new LinkedList<JFrame>();
        frames.addToFront(new JFrame());
        frames.addToFront(new GeoFrame());
        ListStuff.displayFrames(frames);
    }
}