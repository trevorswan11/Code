...
public class LinkedListTester {
    ...
    public void testInsertAfter() {
        ...
        assertEquals("Inserting on end of list. Checking new next node", new Integer(3),
                listNode.getNext().getElement());
        assertEquals("Checking next's next", null, listNode.getNext().getNext());
        listNode.insertAfter(2);
        assertEquals("Inserting in middle of list. Checking new next node", new Integer(2),
                listNode.getNext().getElement());
    ...
    public void testContains() {
        ...
        LLNode<Integer> testOne = new LLNode<>(1, null);
        listTwo.setFirstNode(testOne);
        assertTrue(listTwo.contains(1));
        assertFalse(listTwo.contains(2));

        // List with many things
        LinkedList<Integer> listThree = new LinkedList<>();
        listThree.addToFront(1);
        listThree.addToFront(2);
        listThree.addToFront(3);
        assertTrue(listThree.contains(3));
        assertTrue(listThree.contains(2));
        assertTrue(listThree.contains(1));
        assertFalse(listThree.contains(4));
    }
